...
import { FiPackage, FiClock, FiCheckCircle, FiShoppingBag, FiTruck } from "react-icons/fi";

export default function ProfileOrders({ orders }: { orders: any[] }) {
  
  // Fake Tracking Logic based on Date
  const getStatusStep = (date: string) => {
      const daysPassed = (new Date().getTime() - new Date(date).getTime()) / (1000 * 3600 * 24);
      if (daysPassed > 5) return 3; // Delivered
      if (daysPassed > 2) return 2; // Shipped
      return 1; // Processing
  };
    ...
    <FadeIn className="space-y-8">
      <div className="flex items-end justify-between px-2">
        <div>
            ...
            <p className="text-gray-500 text-sm font-medium">Track your recent purchases.</p>
        </div>
      ...
      {orders.length === 0 ? (
        ...
        <div className="space-y-6">
            {orders.map((order) => {
                const step = getStatusStep(order.createdAt);
                
                return (
                <div key={order.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 hover:shadow-md transition-all duration-300">
                    ...
                    <div className="mb-8">
                        <div className="flex justify-between mb-2 text-[10px] font-bold uppercase tracking-widest text-gray-400">
                            <span className={step >= 1 ? "text-green-600" : ""}>Processing</span>
                            <span className={step >= 2 ? "text-green-600" : ""}>Shipped</span>
                            <span className={step >= 3 ? "text-green-600" : ""}>Delivered</span>
                        </div>
